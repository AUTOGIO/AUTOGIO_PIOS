...
1. Accept the **Shortcut import** dialog when it opens (`PIOS UniFi Daily Health`)
...
```
shortcuts://run-shortcut?name=PIOS%20UniFi%20Daily%20Health
```
...
```bash
FOLLOWUP_DELAY_MINUTES=30 ~/AUTOGIO_PIOS/networking/unifi_baseline/scripts/pios_reminder_action.zsh
```
...
```bash
~/AUTOGIO_PIOS/networking/unifi_baseline/scripts/pios_reminder_action.zsh
```
...
| `setup/PIOS UniFi Daily Health.shortcut` | Unsigned shortcut source |
| `setup/PIOS UniFi Daily Health.signed.shortcut` | Signed shortcut for import |
| `setup/create_pios_reminders.swift` | EventKit helper (daily repeat) |
| `setup/create_pios_followup_reminder.swift` | One-shot follow-up after each health run |
| `setup/install_pios_health_reminders.zsh` | Full installer |
